...
class RobotController:
    ...
    def inverse_kinematics(
        ...
        for iteration in range(self.ik_max_iterations):
            ...
            if target_quat is not None:
                ...
                # 6D 误差向量
                error = np.concatenate([pos_error, error_rotvec * 0.5])
        ...
        if final_error > self.ik_tolerance * 10:
            raise RuntimeError(
    ...
    def move_to_joints(self, target_joints: np.ndarray, speed: float = 0.5) -> bool:
        ...
        n_steps = max(int(max_diff / (0.002 * speed)), 100)  # 更多步以保证平滑
    ...
    def close_gripper(self, force: float = 200, speed: float = 0.5) -> bool:
        """闭合夹爪
    ...
    def pick(self, target_pos: np.ndarray, approach_height: float = 0.15) -> bool:
        ...
        Args:
            target_pos: 抓取目标位置 (3,)
            approach_height: 接近高度（相对于目标 Z 的偏移量）
        ...
        # 3. 下降到抓取位置
        print(f"[Controller] 下降到抓取位置: {target_pos}")
        if not self.move_to_pose(target_pos):
            return False
    ...
    def place(self, target_pos: np.ndarray, approach_height: float = 0.15) -> bool:
        ...
        # 2. 下降
        print(f"[Controller] 下降到放置位置: {target_pos}")
        if not self.move_to_pose(target_pos):
            return False
